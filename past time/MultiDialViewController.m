...
- (void)viewDidLoad {
    ...
    delimit = 1960;
    for  (int x = 0;x<2013-delimit;x++){
        [_years addObject:[NSString stringWithFormat:@"%i",2013-x]];
    }
    int dialCount = 0;
...
-(void)setMyYearLimit:(int)limit{
    if (delimit>limit){
        for  (int x = 2013-(delimit);x<2013-limit;x++){
            [_years addObject:[NSString stringWithFormat:@"%i",2013-x]];
        }
        
    }
    else if (delimit<limit){
        for  (int x = 2013-delimit;x>2013-limit;x=x-1){
...
- (void)spinToRandomString:(BOOL)preset {
    if (preset && self.presetStrings != nil) {
        ...
        [self.dial3 spinToString:[selectedString substringWithRange:NSMakeRange(3, 1)]];
    }
    else {
        //spin to random
